    ...
    return (
        <div className="flex w-full items-center justify-between">
            ...
            <div className="flex items-center gap-1">
                <Button size={'icon'} variant={'outline'} onClick={() => handlePaginate('previous')} disabled={paginate.current_page <= 1}>
                    <ChevronLeft />
                </Button>

                <Button size={'icon'} variant={'outline'} onClick={() => handlePaginate('first')} disabled={paginate.current_page <= 1}>
                    <ChevronFirst />
                </Button>
                ...
                <Button size={'icon'} variant={'outline'} onClick={() => handlePaginate('last')} disabled={paginate.current_page >= totalPages}>
                    <ChevronLast />
                </Button>

                <Button size={'icon'} variant={'outline'} onClick={() => handlePaginate('next')} disabled={paginate.current_page >= totalPages}>
                    <ChevronRight />
                </Button>
            </div>
        </div>
